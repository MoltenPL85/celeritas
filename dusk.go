...
func (c *Celeritas) TakeScreenShot(pageURL, testName string, w, h float64) {
	...
	page := rod.New().ControlURL(u).MustConnect().MustIgnoreCertErrors(true).MustPage(pageURL).MustWaitLoad()
...
func (c *Celeritas) FetchPage(pageURL string) *rod.Page {
	u := launcher.New().Leakless(false).MustLaunch()
	return rod.New().ControlURL(u).MustConnect().MustIgnoreCertErrors(true).MustPage(pageURL).MustWaitLoad()
}
